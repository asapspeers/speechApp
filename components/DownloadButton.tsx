...
import { useSpeech, useAppDispatch } from '../hooks'
...
export default function DownloadButton(props: Props) {
  ...
  useEffect(() => {
    ...
    if (filepaths.length > 0 && speech) {
      setAudioFiles(filepaths)

      let name = filename(speech.uri) || ''
  ...
  }, [audioFiles, files, downloaded, dispatch, speech, filepaths])
  ...
  const saveAudioFile = async () => {
    ...
    const path = base + filename(speech.uri) || ''
